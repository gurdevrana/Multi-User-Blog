...
class BlogFront(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if form_name == "submit_comment":
            ...
            if self.user_owns_comment(prev_comment):
...
class LikeBlog(webapp2.RequestHandler):
    def get(self):
        ...
        if userid and userid != post.username:
            userid = str(userid)
            userid = userid.split("|")[0]
            key = str(key)
            print(key)
...
class EditPost(webapp2.RequestHandler):
    ...
    def post(self, post_key):
        ...
        if userid==post.username: #to check if its user own post or not
            form=self.request.get("hiddeninput")
            form=str(form)
            if form=="editpost" :
                subject = self.request.get("subject")
